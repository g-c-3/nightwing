...
//
// Unit tests for src/eval/{eval,psqt,score}.h — the Phase 2 baseline
// evaluation (material + tapered piece-square tables). Positions are
// built via FEN (fen.h) or Position::place_piece() directly, matching
// the style of movegen_tests.cpp / fen_tests.cpp.
...
TEST_CASE("evaluate: material dominates a full extra queen", "[eval]") {
    ...
    // swing (a few tens of centipawns) can't erase that, so this bound
    // is a safe correctness check, not a tuned expectation.
...
TEST_CASE("evaluate: a bare kings position stays within a small bound", "[eval]") {
    ...
    // (0) and only king psqt terms in play, the score can't be large.
