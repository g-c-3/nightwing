...
[[nodiscard]] double play_one_match_game(const eval::MaterialWeights& weights_white,
                                          ...
                                          const eval::EvalWeightsOverride* eval_weights_white,
                                          const eval::EvalWeightsOverride* eval_weights_black,
    ...
    for (int ply = 0; ply < config.max_plies; ++ply) {
        ...
        if (ply < config.random_opening_plies) {
            ...
            const eval::EvalWeightsOverride* eval_weights_to_move =
                (pos.side_to_move == Color::White) ? eval_weights_white : eval_weights_black;
            ...
            const search::SearchResult result =
                search::search_fixed_depth(pos, config.search_depth, {}, &weights_to_move,
                                            eval_weights_to_move, threads_to_move);
...
MatchResult play_match(const eval::MaterialWeights& weights_a, const eval::MaterialWeights& weights_b,
    ...
    for (int i = 0; i < config.num_games; ++i) {
        ...
        const eval::EvalWeightsOverride* eval_weights_white =
            a_plays_white ? config.eval_weights_a : config.eval_weights_b;
        const eval::EvalWeightsOverride* eval_weights_black =
            a_plays_white ? config.eval_weights_b : config.eval_weights_a;
        ...
        const double result_white_perspective =
            play_one_match_game(weights_white, weights_black, eval_weights_white, eval_weights_black,
                                 threads_white, threads_black,
