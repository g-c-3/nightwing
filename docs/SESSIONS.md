...
## 2026-08-13 — Session 8: Eval + fixed-depth search — Phase 2's first two items done

**What was built:** Phase 2's first two ROADMAP.md items in one session.
...
- `src/CMakeLists.txt` / `tests/CMakeLists.txt` — wired `eval/psqt.cpp`, `eval/eval.cpp`, `search/search.cpp` into `nightwing_lib`, and `eval_tests.cpp`, `search_tests.cpp` into the test binary.
- Verified locally (this session, ahead of a real CI run): both new `.cpp` files compile at `-O3 -Wall -Wextra -Wpedantic` with zero warnings. All `eval_tests.cpp` and `search_tests.cpp` assertions were also run against the real `board`/`fen`/`movegen` code (g++ 13, `-fsanitize=address,undefined`) via standalone harnesses mirroring every Catch2 case — all passed, including exact expected numeric outputs (`evaluate(start_position()) == 0`; depth-1 search nodes == 20; fool's-mate FEN → null move, score == `-32000`; K+Q-vs-K stalemate FEN → null move, score == `0`; back-rank mate-in-1 FEN → `a1a8`, score == `31999`). Not a substitute for the real CI matrix run (Catch2/CMake weren't available to actually drive in this sandbox), but a strong correctness signal ahead of it — flagged here as this session's one test-risk item: **confirm `ctest` is fully green on the actual CI run after committing both eval and search**, since this was checked by equivalent hand-built harnesses, not the committed `nightwing_tests` binary itself.
...
**Decisions made:** Logged in DECISIONS.md — (1) PSQT source (Michniewski's Simplified Evaluation Function, king-only tapered initially, mirror-derived Black tables, transcription cross-check); (2) `Score` as a plain struct rather than packed-int, and full-recompute `evaluate()` rather than an incremental make/unmake accumulator, both scoped as deliberate Phase 2 simplifications; (3) negamax-form search with a fixed `kInfinity` bound (not `numeric_limits::max()`, to avoid negation UB on `INT_MIN`) and ply-decaying mate scores, with fixed-depth-search's horizon-effect limitation (mate exactly at the search horizon isn't detected) explicitly documented rather than left as a surprise; (4) the `search_tests.cpp` init-convention bugfix above, plus a process note on why the sandbox harness missed it and how to avoid the same blind spot for future test files; (5) enabled MSVC's native ASan (`/fsanitize=address`) on the `windows-latest` Debug CI leg (CMakeLists.txt), closing one of the four previously-uncovered sanitizer legs — ASan-only, no UBSan-equivalent on MSVC, and unverified beyond doc-level review since no MSVC toolchain was available in the sandbox (see DECISIONS.md's verification caveat).

**Next session start point:** Phase 2 continues: iterative deepening (wraps `search_fixed_depth()`'s core loop with increasing depth, most likely restructured to share state/keep a running best move across depths — check `src/search/search.h` before deciding whether to extend it in place or add `src/search/iterative_deepening.h/.cpp`) — the next unchecked ROADMAP.md item. Two things need confirming from real CI before that work starts, neither verifiable from this sandbox: (1) the corrected `tests/search_tests.cpp` — all 6 configs should now be green; (2) the new `windows-latest, Debug` CMakeLists.txt change — specifically check that leg didn't fail to *build or link* (a flag-level MSVC mistake would show up there, not as a test failure). If `windows-latest, Debug` fails at build/link (not test) time, the likely suspects are the `/RTC1` removal or the `/INCREMENTAL:NO` linker flag — see DECISIONS.md's entry for the exact reasoning to revisit. Say "Continue" or "Start" to proceed.
