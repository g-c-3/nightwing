...
| Transposition table | One persistent, engine-lifetime table (`src/uci/uci.cpp`'s `run()`) shared by every ordinary `go`/`go ponder` call in the real UCI binary — rebuilt only when `setoption name Hash` genuinely changes its size, `clear()`-ed (not rebuilt) on `ucinewgame` (DONE, Session 96 — see `src/search/tt.h`'s own LIFETIME NOTE and docs/DECISIONS.md, 2026-09-09 (1); corrected here, 2026-09-26, after this row was found still describing the pre-Session-96 state). Direct, external-tt-less calls into the search layer (this project's own tests, `nightwing_bench`/`nightwing_selfplay`/`nightwing_sprt`) still default to a fresh, private, per-call table instead, since none of those need cross-call persistence — both lifetimes are real, intentional, permanent options selected via `external_tt`, not one interim placeholder waiting on the other. Power-of-2 sized, Zobrist hashing, age + depth replacement scheme; cache-line-aligned entries (16 bytes, 4 entries per 64-byte line), explicit prefetch on probe. Genuinely lock-free for concurrent Lazy SMP use as of Session 72 — CPW "Shared Hash Table" XOR-checksum technique, two atomic 64-bit words per entry, no locks (`src/search/tt.h`'s THREAD-SAFETY NOTE) |
| Multithreading | Lazy SMP (Phase 7) — helper threads sharing the TT (`search_iterative_deepening()`'s `num_threads` parameter, Session 71) and the TT's lock-free redesign (Session 72) are both landed. Helper threads are diversified, not identical copies of the main thread's loop (Session 98): each gets a `helper_id`-derived staggered starting depth, an occasional 2-ply jump instead of 1 for odd-numbered ids, and its own move-ordering tie-break variant, propagated through that helper's entire subtree. The UCI `Threads` option (`setoption name Threads value <N>`, clamped, session-lifetime) has been implemented since Session 74 — corrected here, 2026-09-26, after this row was found still describing it as open (docs/DECISIONS.md, 2026-09-26 (7)) |
...
**A real compiler/build toolchain IS available in the assistant's own sandbox as of 2026-09-23 — this supersedes every prior session's "no compiler toolchain available in this environment" caveat scattered through docs/DECISIONS.md and docs/SESSIONS.md.** `g++`/`gcc` (GNU 13.3.0) and Python 3 are present out of the box; `cmake` is not preinstalled but installs cleanly via `apt-get install -y cmake` (the sandbox's network allowlist already covers `archive.ubuntu.com`/`security.ubuntu.com`, the Ubuntu package mirrors this needs). The sandbox's network allowlist also covers `github.com`/`codeload.github.com`, so CMake's own `FetchContent` successfully pulls Catch2 from GitHub during configuration — a full `cmake -G "Unix Makefiles" .. && cmake --build .` of this actual repository, EVERY target including `nightwing_sprt`, and a full `ctest` run of the WHOLE test suite (a specific total was named here as of 2026-09-23; deliberately not repeated, since it grows every session that adds tests and would only go stale again — `docs/SESSIONS.md`'s own latest entry has the current count), all genuinely succeed in this sandbox, not just in GitHub Actions.

**What this changes going forward:** a session touching `src/` should build the affected target(s) and run at least the relevant `ctest` subset (ideally the full suite, time permitting) before presenting a file as a deliverable, rather than defaulting straight to "verified by inspection, no compiler available" the way every session before this one did. This is a strictly stronger verification standard than before, not a relaxation of anything — CORE RULES' "GitHub Actions handles ALL building — never ask the user to run cmake/make/g++ locally" is about what the mobile-only PERSON is asked to do, and is unaffected; this note is about what the ASSISTANT can now do for itself, in its own sandbox, before shipping. `nightwing_sprt` itself builds and runs, but a genuine SPRT verdict still needs many real games against a reference build (real wall-clock time, not a quick sandbox check) — a fast local `ctest` pass is a correctness gate, not a substitute for that.

**Why this was never noticed sooner:** every prior session's own stated toolchain check apparently stopped at the first `which cmake`-equivalent failure (`cmake` genuinely isn't preinstalled) without trying `apt-get install` against the mirrors the sandbox's network policy already permits, or without trying `g++` directly for a narrower, non-CMake compile/link check. Confirmed as a real, reproducible capability during ROADMAP.md Priority Fixes (2026-09-22) item 4's own work (docs/DECISIONS.md, 2026-09-23 (2)/(3)) — used there to build the actual repository, apply that session's own `search.cpp` changes on top, and both build and run the full test suite for real, which is how a genuine, previously-undetected pre-existing test flake (docs/DECISIONS.md, 2026-09-23 (3)) got caught and fixed rather than shipped or missed.
