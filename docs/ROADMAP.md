...
- [ ] Tuned weights committed, before/after strength comparison logged — strength-comparison infrastructure now built (`tuner::match`, `src/tuner/match.h`/`.cpp`/`match_main.cpp`, `nightwing_match`; required threading `eval::MaterialWeights` all the way through `search::search_fixed_depth()`/`negamax()`/`quiescence()`, not just `eval::evaluate()`, as a prerequisite), verified working end to end (`nightwing_selfplay` → `nightwing_tune` → `nightwing_match`). Still to do: an actual large-scale self-play + tuning run (thousands of games, not this session's smoke tests), and committing the resulting weights with a logged before/after comparison. See docs/DECISIONS.md.
