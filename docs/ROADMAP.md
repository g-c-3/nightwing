...
- [ ] Tuned weights committed, before/after strength comparison logged — strength-comparison infrastructure built and dispatchable via CI (`tuning-pipeline` `workflow_dispatch` job, `.github/workflows/ci.yml`) as of Session 61. That first real production run (5000 self-play games, 200 iterations, 400 match games) surfaced a tuner bug — pawn value collapsed to ~21% of its starting value (a classic Texel-tuning scale-degeneracy artifact, `sigmoid_scale` being fixed rather than fit from data) and the resulting weights scored no better than the untuned defaults in the match comparison. Fixed in Session 62: `pawn_mg`/`pawn_eg` are now anchored (`tuner::MaterialParameterRef::anchored`, `src/tuner/tune.h`/`.cpp`) — held exactly fixed while the other eight `MaterialWeights` fields still tune normally, removing the flat scaling direction that caused the drift. Verified end to end (smoke-scale pipeline run before/after; full test suite green, 325 cases/52,849 assertions including two new tuner tests). Still to do: re-dispatch the `tuning-pipeline` workflow now that the fix is in place (same production-scale defaults), review the new `tuned_weights.txt`/`match_result.txt`, and — if the tuned weights come out ahead this time — hand-transcribe into `eval/psqt.h`'s `kPawnValue`/etc. constants with the before/after comparison logged here and in DECISIONS.md. See docs/DECISIONS.md, 2026-08-31 (1) and (2) entries.
...
---
**Phase 2 complete. Phase 3 complete** (its former "Pondering" item moved to Phase 7 — see above). **Phase 4 complete.** **The Priority Fixes section above is complete** (external code review, 2026-08-25 — both mid-search time checks and UCI `info` output done). **Current: Phase 5 — Eval Expansion & Tuning** — Mobility eval, King safety, Bishop pair/rook-on-open-or-semi-open-file/rook-on-7th-rank, Knight outposts, Space evaluation, Threats evaluation, King tropism, Trapped piece penalties, Tempo bonus, Material imbalance table, Eval cache, All terms as named tunable constants, and the Texel/SPSA tuner module (material weights only so far) are all done; the final item, "Tuned weights committed, before/after strength comparison logged," has its infrastructure built and dispatchable via CI (`tuning-pipeline` in `.github/workflows/ci.yml`) — its first real production run surfaced and Session 62 fixed a tuner scale-degeneracy bug (pawn value now anchored, not tuned) — still needs that workflow re-dispatched with the fix in place, its artifacts reviewed, and (if the tuned weights come out ahead) committed with the comparison logged — that's next.
