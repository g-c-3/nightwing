...
- [ ] Lock-free TT for concurrent access — NOTE (Session 71): the TT is already safe for Lazy SMP's concurrent use as of the item above, via coarse striped locking (`src/search/tt.h`'s THREAD-SAFETY NOTE) — this item is specifically about replacing that interim locking with a true lock-free scheme (e.g. XOR-checksum entries + atomic word-sized access), not about first-time thread-safety, which is already done.
