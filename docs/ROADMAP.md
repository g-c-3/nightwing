...
- [ ] Material-signature classifier: detect endgame material buckets at each node, route to specialized endgame reasoning when matched — CLASSIFICATION half done (Session 64): `eval::classify_endgame()` (`src/eval/endgame.h`/`.cpp`) recognizes six buckets by piece count/side (KPK, KRK, KBNK, RookEndgame, OppositeColoredBishops, KnightVsBishop — one per later Phase 6 item below), fully tested (`tests/endgame_tests.cpp`, 18 test cases). NOT yet wired into `eval::evaluate()` or search/ — deliberately: there's no "specialized endgame reasoning" for it to route TO yet, since none of the items below exist. Each later item in this phase is expected to consult `classify_endgame()`'s result once it lands; this checkbox stays open until at least one of them actually does.
...
---
**Phase 2 complete. Phase 3 complete** (its former "Pondering" item moved to Phase 7 — see above). **Phase 4 complete.** **The Priority Fixes section above is complete** (external code review, 2026-08-25 — both mid-search time checks and UCI `info` output done). **Phase 5 complete** (2026-08-31, Session 63 — see that phase's own final-item note above for the tuning-run history). **Current: Phase 6 — Endgame Knowledge** — the material-signature classifier's classification half is done (Session 64, `eval::classify_endgame()`), not yet wired to anything; K+P theory, rook endgame patterns, minor piece endgames, fortress detection, zugzwang-aware search shaping, and the KPK/KRK/KBNK base heuristics are all still open, each expected to consult the classifier once it lands.
