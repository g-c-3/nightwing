...
- [ ] Texel/SPSA tuner module (self-play data generation + gradient descent) — self-play data generation half done (new `tuner::` module, `src/tuner/selfplay.h`/`.cpp`/`selfplay_main.cpp`, `nightwing_selfplay` executable); gradient-descent tuning loop over eval's named constants still to do — see docs/DECISIONS.md for why the two halves are split across sessions
