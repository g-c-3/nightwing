...
- [ ] Tuned weights committed, before/after strength comparison logged — strength-comparison infrastructure now built (`tuner::match`, `src/tuner/match.h`/`.cpp`/`match_main.cpp`, `nightwing_match`; required threading `eval::MaterialWeights` all the way through `search::search_fixed_depth()`/`negamax()`/`quiescence()`, not just `eval::evaluate()`, as a prerequisite), verified working end to end (`nightwing_selfplay` → `nightwing_tune` → `nightwing_match`). A `tuning-pipeline` `workflow_dispatch` job (`.github/workflows/ci.yml`) now runs this full pipeline on GitHub-hosted infrastructure and uploads `training_data.txt`/`tuned_weights.txt`/`match_result.txt` as a build artifact, per this codebase's own "GitHub Actions handles ALL building/running of anything compute-heavy" convention (already documented in `src/tuner/selfplay_main.cpp`'s/`tune_main.cpp`'s/`match_main.cpp`'s own header comments) — this closes the "where does a real large-scale run actually happen" gap without needing a local build. Still to do: the workflow needs to actually be triggered (a thousands-of-games run, not this session's local smoke tests) with its resulting `tuned_weights.txt`/`match_result.txt` artifacts brought back and reviewed, and — if the tuned weights come out ahead — hand-transcribed into `eval/psqt.h`'s `kPawnValue`/etc. constants with the before/after comparison logged here and in DECISIONS.md. See docs/DECISIONS.md.
