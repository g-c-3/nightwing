...
## 2026-08-13 — CMake: enable MSVC's native ASan on Windows Debug (ASan-only, no UBSan-equivalent)

**Decision:** Top-level `CMakeLists.txt`'s Debug-config branch now sets MSVC-specific flags (`/fsanitize=address` on the compiler, `/INCREMENTAL:NO` on the linker) instead of leaving the `windows-latest` Debug leg with zero sanitizer coverage. This is a full override of `CMAKE_CXX_FLAGS_DEBUG`/`CMAKE_EXE_LINKER_FLAGS_DEBUG` for MSVC (mirroring the existing non-MSVC branch's shape), not an append — MSVC's default Debug flags include `/RTC1` (runtime checks), which Microsoft's own AddressSanitizer docs list as incompatible, so it's dropped rather than left alongside the sanitizer; incremental linking is documented as similarly incompatible and is explicitly disabled.

**Rationale:** Before this change, 4 of the 6 CI matrix legs (`windows-latest` × both configs, and both `Release` legs on every OS) ran with zero sanitizer instrumentation — any UB that didn't happen to crash outright would silently pass on those legs and only get caught on `ubuntu-latest`/`macos-latest` Debug. MSVC has shipped a real, actively maintained native ASan (`/fsanitize=address`, GA since VS 16.9, using the same LLVM ASan runtime as GCC/Clang) for long enough that leaving it off was a widening, not a stable, gap. This closes one of those four legs down to ASan-only coverage.

**Known limitation, deliberately not worked around:** MSVC has no UBSan equivalent, so Windows Debug will only catch memory-safety classes of bug (buffer overflow, use-after-free, stack corruption) via ASan, not undefined-behavior classes like signed overflow or an out-of-range shift — the exact class of bug that this session's `search_tests.cpp` fix hit (`>> (64 - 0)`, a UBSan-only diagnostic). On Windows, that specific bug would likely still have surfaced as ASan catching the resulting null-pointer dereference (a memory-safety symptom one step downstream of the actual UB), just without the precise root-cause line UBSan gave on Linux/macOS. This is treated as an acceptable, honestly-documented gap rather than something to fake — there's no drop-in UBSan-equivalent for MSVC to reach for.

**Verification caveat:** No MSVC toolchain is available in the sandbox this was written in, so this change is unverified beyond a syntax-level review against Microsoft's documented flag set (`/fsanitize=address`, `/RTC1` incompatibility, `/INCREMENTAL` incompatibility) — **flagged as a test-risk item: watch the `windows-latest, Debug` CI leg specifically on the next push**, since a flag-level mistake here would only surface as a Windows-specific build or link failure, not something catchable by this session's Linux-based verification harnesses.

**Alternatives considered:**
- Leave Windows Debug/Release and all Release legs with no sanitizer coverage — this was the status quo; rejected once a concrete, real bug (the `search_tests.cpp` init-order crash) demonstrated the coverage gap wasn't hypothetical.
- Also enable ASan on Release builds (Microsoft's docs note `/fsanitize=address` is compatible with all optimization levels) — rejected for now; matches the existing non-MSVC policy of sanitizers-in-Debug-only, and adding it to Release everywhere is a separate, larger CI-cost/noise tradeoff worth its own decision if pursued later, not bundled into this narrower fix.
- Third-party LLVM-based ASan port for Windows (clang-cl + `clang_rt.asan-*.lib`) instead of MSVC's native `cl.exe` integration — rejected; would mean switching the Windows CI leg's compiler entirely, a much larger change than closing a coverage gap warrants.
